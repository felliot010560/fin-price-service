...
/**
 * This is the controller that sends data to the front end--and only to the
 * front end. Communicating with other backend services (like the trading
 * service) is done via the classes in
 * {@link com.aleatory.price.backend.messaging}.
 * 
 * When using websockets as the backend transport, care must be taken not to
 * send messages twice, since the same messsaging system (websockets) will be
 * used for both the frontend and backend. When using other backend transport
 * (currently only Redis), you must send to both the frontend and backend
 * independently; this will generally entail listening for the same events as in
 * this controller and sending to the correct topics to ensure that the other
